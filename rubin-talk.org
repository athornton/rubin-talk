...
* A Brief Overview of the Rubin Observatory (2023)
...
We expect to begin science operations in late 2024.
...
That's the weather station on the left.  It still hasn't been rebuilt.
...
+ The improvements in the distance ladder from lensing and SN1ae can let
  ...
  proper distance) more   precisely.  That will let us rule out some
  dark energy theories.
+ And of course if the Hubble Parameter is /not/ (statistically) constant
...
users with the notebook platform.  That's on our development environment
now and should move into production within a few weeks.
